...
public class AnalyserService {
    ...
    void pulseProcessing(Sensor sensor) {
        ...
        SensorLastValues sensolrLastValues = sensorRepository.findById(sensor.id).orElse(null);
        if (sensolrLastValues == null) {
            LOG.debug("for sensor id {} not found record in redis", sensor.id);
            sensolrLastValues = new SensorLastValues(sensor.id);

        } else {
            int lastValue = sensolrLastValues.getLastValue();
        ...
        sensolrLastValues.setLAstValue(sensor.value);
        sensorRepository.save(sensolrLastValues);
